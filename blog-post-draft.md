...
tags:
  ...
  - compiler-design
  - sovereign-ai
slug: knowledge-compiler-compiling-human-knowledge-into-static-semantic-artifacts
---
...
**Suggested tags:** `knowledge-compiler`, `RAG`, `GraphRAG`, `knowledge-graphs`, `semantic-search`, `compiler-design`, `AI-infrastructure`, `local-ai`, `sovereign-ai`, `static-sites`, `vector-embeddings`
...
1. **Pipeline architecture flow** — horizontal diagram showing the 9 phases as connected boxes, with pass counts in each phase
2. **Compiler analogy comparison** — side-by-side: software compiler stages vs. knowledge compiler stages
3. **Runtime cost comparison** — bar chart comparing per-query costs: standard RAG vs. GraphRAG vs. compiled knowledge
4. **Knowledge graph visualization** — screenshot from the D3 force-directed graph in the web app showing entity/document/cluster relationships
5. **IR store architecture** — diagram showing the relationships between DocAST, SectionGraph, KnowledgeGraph, ConceptGraph, ClusterGraph, and EmbeddingStore
6. **TF-IDF vs. OpenAI embedding comparison** — showing the fallback chain: both produce useable vectors, but with different quality
7. **SimHash dedup visualization** — before/after showing duplicated documents being flagged

**Suggested screenshots from the repository:**
1. The D3 knowledge graph visualization from `apps/web/src/app/graph/`
2. The concept hierarchy tree from `apps/web/src/app/concepts/`
3. The CLI compiling a corpus (terminal output showing "113 documents, 192k concepts, 1.5s")
4. The dashboard statistics cards from the web app home page
5. The cluster view showing color-coded topic groups

**Suggested code snippet locations:**
- TF-IDF keyword extraction (packages/core/src/compilers/analysis.ts)
- SimHash implementation (packages/core/src/compilers/optimization.ts)
- Knowledge graph construction (packages/core/src/compilers/graph.ts)
- IR type definitions (packages/ir/src/knowledge.ts, doc-ast.ts)
- Binary embedding format (packages/artifacts/src/binary.ts)
- Pipeline orchestration (packages/core/src/pipeline.ts)
- Embedding fallback (packages/core/src/compilers/embedding.ts)
